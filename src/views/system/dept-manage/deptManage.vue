...
<template>
  <div class="search">
    <Card>
      <Row class="operation">
        <Button type="primary" @click="add" icon="md-add">添加子部门</Button>
        <Button type="info" @click="addRoot" icon="md-add">添加一级部门</Button>
        <Button type="warning" icon="ios-trash" @click="delAll">批量删除</Button>
        <Button type="success" @click="getParentList" icon="ios-loading">刷新</Button>
      </Row>
      <Row type="flex" justify="start" class="code-row-bg">
        ...
        <Col span="9">
          <Form ref="form" :model="form" :label-width="85" :rules="formValidate">
            <FormItem label="上级部门" prop="parentTitle">
              <Poptip trigger="click" placement="right-start" title="选择上级部门" width="250">
                ...
                <div slot="content" style="position:relative;min-height:5vh">
                  <Tree :data="dataEdit" @on-toggle-expand="expandDept" @on-select-change="selectTreeEdit"></Tree>
            ...
            <FormItem label="部门负责人" prop="leader">
              <Select
                :loading="userLoading"
    ...
    <Modal :title="modalTitle" v-model="modalVisible" :mask-closable="false" :width="500">
      <Form ref="formAdd" :model="formAdd" :label-width="85" :rules="formValidate">
        ...
        <FormItem label="部门名称" prop="deptName">
          <Input v-model="formAdd.deptName"/>
        </FormItem>
        <FormItem label="排序值" prop="sort">
        ...
        <FormItem label="是否启用" prop="enabled">
          <i-switch size="large" v-model="formAdd.enabled" :true-value="1" :false-value="0">
            <span slot="open">启用</span>
            <span slot="close">禁用</span>
          </i-switch>
        </FormItem>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      expandDept(v) {
        // 判断展开子节点
        this.checkDepTree(v.children)
      },
      // 判断子节点
      checkDepTree(depData) {
        let that = this
      ...
      submitAdd() {
        this.$refs.formAdd.validate(valid => {
          if (valid) {
            ...
            if (this.formAdd.btnType === null) {
              this.formAdd.btnType = "";
            }
